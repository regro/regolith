...
def test_builder(bm, db_src, make_db, make_mongodb, monkeypatch):
    ...
    if bm == "reimb" or bm == "recent-collabs":
        ...
        subprocess.run(["regolith", "build", bm, "--no-pdf",
                        "--kwargs", "facility:tbd"], check=True, cwd=repo)
        # subprocess.run(["regolith", "build", bm, "--no-pdf",
...
def test_builder_python(bm, db_src, make_db, make_mongodb,
    ...
    expected_base = os.path.join(os.path.dirname(__file__), "outputs")
    for root, dirs, files in os.walk("."):
        glob_list = os.listdir(os.path.join(expected_base, bm, root))
        files_list = [file for file in glob_list if os.path.isfile(file)]
        for file in files_list:
            if file not in files:
                print(f"Expected test file {file} not generated by the builder")
                assert False
        for file in files:
            if file in os.listdir(os.path.join(expected_base, bm, root)):
                html_tex_bool = False

"""Helper for setting a task as finished or deleting the task by entering the mark of a certain task. The mark can be found by regolith helper l_todo -v.
"""
...
def subparser(subpi):
    subpi.add_argument("mark", help="the mark of the to_do task you want to set as finished, can be found by regolith helper l_todo -v ",
                       default=None)
    subpi.add_argument("-d", "--delete", action="store_true",
...
class TodoUpdaterHelper(DbHelperBase):
    ...
    def db_updater(self):
        ...
        if len(pdocl) == 0 :
            ...
            if not rc.delete:
                # set the to-do task as finished
                for member in coll:
                    if key == member["_id"]:
                        todo_update=[]
                    ...
                    if key == member["_id"]:
                        ...
                        for todo_item in member["todos"]:
                            if todo_item['mark'] == int(rc.mark):
                                ...
                                todo_item['mark'] = new_mark
                                todo_update.append(todo_item)
                                new_mark+=1
                        rc.client.update_one(rc.database, rc.coll, {'id': rc.id},  {"todos": todo_update},
                                         upsert=True)
                        print(f"The task ({task_name}) is deleted.")
                        break

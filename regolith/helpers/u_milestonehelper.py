...
def subparser(subpi):
    ...
    subpi.add_argument("-u", "--milestone_uuid",
        ...
        if rc.projectum_id:
            ...
            if target_mil:
                for prum in target_mil:
                    ...
                    for milestone in milestones:
                        if milestone.get('uuid') == rc.milestone_uuid:
                            upd_mil.append(milestone)
                            all_miles.append(milestones)
                    if upd_mil:
                        pid = prum.get('_id')
                        id.append(pid)
            if target_del:
                for prum in target_del:
                    if prum.get('_id') == rc.milestone_uuid:
            ...
            if target_ko:
                for prum in target_ko:
                    if prum.get('_id') == rc.milestone_uuid[2:]:

        ...
        pdoc.update(
            {
                "deliverable": {
                    "due_date": due_date,
                    "audience": [
                        "Who will use the software or read the paper? Your target audience. e.g., beginning grad in chemistry"
                    ],
    ...
    def insert_checklists(self, pdoc, now):
        ...
        presubmission_checklist = [
            (
                ...
                "Create Inkscape graphics (Inkscape is preferrable over ppt) for the slides and place in a ``figures`` directory in the slides directory. These may then be used either in beamer or ppt. Iterate with PI to convergence. (to get started with Inkscape download and install it, then run the program and navigate to Help-->Tutorials.  The first two ('Basic' and 'Shapes') should probably be enough for someone to get basic functionality.).",
            ),
            (
                "Create slides",
                "Create a 'slides' folder in the paper repo or a Google slides deck for a series of talk slides. Iterate the slide skeleton with PI to convergence. (For a beamer template: https://gitlab.thebillingegroup.com/talks/beamerTalkTemplate).",
            ),
            (
                "Create a highlight slide",
                "Create a 'highlight' folder in the paper repo. Create a single 'highlight' slide that describes the result following NSF/DOE guidelines. Place it in the 'highlight' folder. Iterate with PI to convergence (highlight templates and examples can be found in https://gitlab.thebillingegroup.com/papers/highlights)",
            ),
            (
                "Create public-summary",
                "Create a public summary in a text file. Place it in the 'highlight' folder. Iterate with PI to convergence. (The kudos template and example can be found at https://docs.google.com/document/d/1j4ZsM8zS_nZo03s7T48uwzDbh8xTPksAQM3ZLgJ-g-Y/edit?usp=sharing).",
            ),
            (
                "Add links to urls with supplementary info in the prum",
                "Add url links to raw or analyzed data and code that was used in the data analysis, and anything else, in gitlab or elsewhere into the 'supplementary_info_urls' field in the prum",
            ),
        ]
        ...
        submission_checklist = [
            (
                ...
                "If it will be submitted to a preprint server such as arxiv, put it in the citations collection in a public database, if the paper is not on a preprint server put the entry in the citations collection on a non-public (e.g., group) database Check, double check, and triple check that the tag for the grant is correct and the tags for the facilities are correct. Any questions, ask PI.",
            ),
            (
                "Create public facing supplementary information as needed",
                "Ask first before making things public, but any code or data that we want to publish as supplementary info should be put in a place where it can be made public or at least available to referees (e.g., in a repo on GitHub).",
            ),
            (
                "Make sure the public facing SI info has the right license",
                "check with the PI for the best license to put on this, but it will likely be some kind of creative commons license",
            ),
            (
                "Put url links to all open shared SI data repositorities into the 'supplementary_info_urls' field in the entry for this paper in the citations collection.",
                "the url links in supplementary_info_urls in the projecta collection are to internal data- and code- stores so future group members can find things.  The fields in the citations collection should be public facing urls",
            ),
            (
                "Check the author list",
                "Check the author list. Last chance to check for missing authors. Does each author agree to submit to the specific journal? Make sure all authors have approved submission.",
            ),
            (
                "Check publisher accounts",
                "Check that all of the authors have accounts for the publisher you are submitting to (if possible) and that they have their ORCID IDs associated with their accounts (ie. for ACS, authors need to link their Paragon accounts with their ORCID IDs).",
            ),
            (
                "Check institution",
            ...
            (
                ...
                "Are beamlines, grants, fundings properly acknowledged at the end of the paper? Double check this with PI and use the ackno statements in the Group Google group.",
            ),
            (
            ...
            (
                ...
                "Check that the figure axis labels are correctly labeled. Make sure it doesn't say G when F is plotted. Make sure the units are correct. Make sure it says 'G (A^-2)' and NOT 'G(r)' (common mistake).",
            ),
            (
                "Check table captions",
                "Check the table caption is correct. Are all the items in the table properly defined in the caption. If it is a crystal structure, are the space group and special positions mentioned in the caption? Is all the info correct?",
            ),
            ...
            (
                ...
                "Check references to the all figures and tables. Does reference to Figure 4 refer to the right figure for example.",
            ),
            (
                "Check references",
                "Go through the references and find all the errors. Correct errors in the bibliographic database (citations.yml, or the Zotero collection, for example), not in the local bib file. Did all the journal names compile correctly? Are they all consistently in abbreviated form (or full form if that is the style, though that is rare). Volume, year and page numbers appear for all references? Hard to find errors in these numbers, but when you do, definitely correct the database!",
            ),
            ("Check reference style", "Is reference's style in accordance with journal's requirement?"),
            (
                "Check journal submission requirements",
                "Check the journal submission requirements for cover letters, table of contents pictures, list of referees, etc..",
            ),
            ("Check arxiv", "Check with PI; will the paper be submitted to arXiv?"),
            (
                "Get approval",
                "Get final approval from all authors for the final version of the manuscript, cover letter, referees list, etc., submission to arXiv if appropriate.",
            ),
            (
                "Check open access",
                "Check with coauthors if the paper should be submitted open access.  Do not guess on this one as there may be financial consequences",
            ),
            (
                "Check cover letter",
                "In the cover letter, does it contain editor-in-chief's name and institution (usually at the top left of the letter) ? Is the content of letter concise and eye-catching? Are (three) suggested reviewers' information in the letter?",
            ),
            (
                "Do a trial submission",
                "Go through the online submission process finding out all the questions that you will be asked, but don't actually press submit, and then check with the PI about the right answers",
            ),
            ...
            (
                "Push a tag",
                "If during the submission process you need to make any changes, do it in your local repo and make another commit and push. When the submission is finalized, tag the repo that points to THIS VERSION IS THE SUBMITTED VERSION. create the submission tag.  If the current version of your local repo is the submitted version, type, e.g., `git tag -l` to list previous tags (try and keep the tag name formatting consistent) `git tag -a 20180525PRLsubmitted -m <initial submission to PRL>` `git push origin <tag_name>`.",
            ),
            (
                "Modify tag if needed",
                "If you forgot to tag and made some changes to the repo and  need to point the tag to an earlier version, or want to view all the different tags, or do some other complicated thing, more info about tagging git repos is here: https://git-scm.com/book/en/v2/Git-Basics-Tagging",
            ),
        ...
        resubmission_checklist = [
            (
                "Work on the changes",
                "Make changes to the manuscript in the repo. You don't need to save it to a new name or anything because we can recover the old version by checking out the tagged version.",
            ),
            (
                "Write rebuttal letter",
                "Write a proper rebuttal letter based on reviewers' comments and place in the repo. In this letter address all of the referee's points, one by one. Place a copy of the referee's comments in the repo. Give it a unique filename in case there are more referee comments from later submissions!",
            ),
            (
                "Check author list",
                "This is a good time to check that the author list is correct (don't need to add anyone or remove anyone) and that the acknowledgements have been done correctly, the figures are correct, the figure captions and table captions are correct and all the figures and tables are correctly referenced, and there are no compilation errors. Check all references for errors and update any 'unpublished' references if they have been published.",
            ),
            (
                "Diff the changes",
                "create a diff.pdf file that shows changes to the manuscript between the version in the tag of the previous submission and the current version, and include this in the resubmission.",
            ),
            (
                "Send to coauthors",
                "Send the final version to your coauthors. Tell them you will 'submit on' where is somewhere around 48 hours later and ask for any final corrections etc. from them. Offer them the chance to extend the deadline if they need more time, i.e., write 'if you need more time, lease let me know.' However, it is assumed that all the authors have been involved in the correction process up to this point so they only have to give it one final thought...",
            ),
            ...
            (
                ...
                "If during the submission process you need to make any changes, do it in your local repo and make another commit and push.",
            ),
        ...
        accepted_checklist = [
            ...
            (
                ...
                "If it is a BNL paper (check with PI, but it should acknowledge BNL funding-not just beamtime), send a pdf copy of the accepted version of the paper from the repo to Arlene at BNL to get a BNL publication number.  If you are not sure what this means, ask PI",
            ),
            (
                "Share the proof",
                "When you receive the proofs, share them quickly with all the authors. Request comments back in 24 hours. Proofs should be responded to within 48 hours in normal circumstances.",
            ),
            ...
            (
                "Check acknowledgement",
                "Make sure that all funding and all beamlines used are correctly acknowledged. Usually this is done by the bosses, but more eyes catch more mistakes.",
            ),
            (
                "Update the db entry",
                "In citations.yml, (the reference should have been added during the submission step) double check the grants{}, facilities{}, nb{} field entries. Any questions, ask PI. Put 'to be published' in the note{} section. If it has not been submitted to arxiv before, move the entry from rg-db-group to rg-db-public github repo. Otherwise, it should be at rg-db-public already. Create a PR to merge to the billingeGroup repository for edits if necessary.",
            ),
            (
                "Check db errors",
            ...
            (
                ...
                "If the manuscript style has titles in the references, make sure there are no capitalization or other compilation errors. Again, correct these in the database using {braces} around words where you want to preserve the capitalization as well as on the proof.",
            ),
            (
                "Read the paper",
                "Finally, after you have done all these 'mechanical' checks, read through the paper and try and find any typos or other problems. Resist the temptation to do any rewriting here...you are looking for mispellings and missing or extra words and so on.",
            ),
            (
                "Apply corrections from coauthors",
                "Collect all the corrections from the other authors and add any additional ones to the proof and return it.",
            ),
            ("Email coauthors", "Send an email to your coauthors that this was successfully resubmitted."),
            (
                "Revisit talk slides",
                "Revisit the set of talk slides that summarize the result in a few slides if they need to be updated. Iterate with PI to convergence.",
            ),
            (
                "Revisit the highlight slide",
                "Create a single 'highlight' slide that describes the result following NSF/DOE guidelines. Place it in the 'highlight' folder. Iterate with PI to convergence (highlight templates and examples can be found in http://gitlab.thebillingegroup.com/highlights/highlightTemplate)",
            ),
            (
                "Check the url links to all open shared SI data repositorities in the 'supplementary_info_urls' field in the entry for this paper in the citations collection.",
                "make sure they work and the data are accessible.",
            ),
            (
                "Create web news",
                "Create a web news story for thebillingegroup.com site. Place it in the 'highlight' folder. Iterate with PI to convergence",
            ),
            (
                "Revisit kudos",
        ...
        published_checklist = [
            ...
            (
                "Update db entry",
                "Update citations.yml at rg-db-public github repo with the correct reference information. Commit your edited citations.yml and create a PR to merge to the billingeGroup repository.",
            ),
            (
                "Check db entry",
            ...
            (
                ...
                "grant{} lists just the billinge-group grants that appeared in the acknowledgement section. They have standard abbreviations that are listed at the top of the citations.yml file, e.g., fwp, EFRC10, etc.. Use the right standard or the whole system becomes broken! If not sure.....ask PI. List all grants in a comma-separated list.",
            ),
            (
                "Check the facility in the db entry",
                "facility{} is every beamline that was used for data collection. Again, use the standard abbreviations at the top of the file. Use two levels of granularity for each, so X17A would be: 'nsls, x17a', if X17A and X7B were used it would be 'nsls, x17a, x7b' and so on.",
            ),
            (
                "Check the nb in the db entry",
                "nb is some other tags, also listed at the top of the file. 'art' for a regular article and 'hilite' if it is one of our top top papers are the most common.",
            ),
            (
                "Check the tags in the db entry",
                "tags should reflect the content so we can automatically build reading lists by subject.  Most papers are PDF papers, so no need to say pdf, be more targeted.",
            ),
            (
                "Add the url link to the SI at the journal to the 'supplementary_info_urls' field in the entry for this paper in the citations collection.",
                "Don't necessarily remove the others if they are still open and public.  It is also ok to put URLs to our data repositories as part of the SI in the paper",
            ),
            (
                "Check db errors",

...
from regolith.tools import (
    ...
    get_pi_id, get_uuid, fragment_retrieval
)
...
class TodoAdderHelper(DbHelperBase):
    ...
    def db_updater(self):
        ...
        if rc.milestone_uuid:
            ...
            target_prum = \
            fragment_retrieval(self.gtx["projecta"], ["milestones"],
                               rc.milestone_uuid)
            if not target_prum:
                raise RuntimeError(
            ...
            target_mil = [mil for mil in target_prum['milestones'] if
                          rc.milestone_uuid in mil.get('uuid')]
            ...
            if len(target_mil) > 1:
                ...
                print(f"The milestone uuid {rc.milestone_uuid} in projectum {target_prum.get('_id')} has been updated in projecta.")

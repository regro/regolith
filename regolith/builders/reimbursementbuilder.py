...
class ReimbursementBuilder(BuilderBase):
    ...
    def construct_global_ctx(self):
        ...
        gtx["expenses"] = all_docs_from_collection(rc.client, "expenses")
    ...
    def excel(self):
        ...
        for ex in gtx['expenses']:
            ...
            if ex.get('expense_type', 'business') == 'business':
                ws['G10'] = 'X'
                ws['L11'] = mdy({k: ex.get('begin_' + k)} for k in
                                ['month', 'day', 'year'])
                ws['O11'] = mdy({k: ex.get('end_' + k)} for k in
                                ['month', 'day', 'year'])
            else:
                ws['G7'] = 'X'
                ws['L8'] = mdy({k: ex.get('begin_' + k)} for k in
                               ['month', 'day', 'year'])
                ws['O8'] = mdy({k: ex.get('end_' + k)} for k in
                               ['month', 'day', 'year'])
            payee = fuzzy_retrieval(gtx['people'], ['name', 'aka', '_id'],
                                    ex['payee'])
            ...
            for i, item in enumerate(ex['itemized_expenses']):
                ...
                ws.cell(row=r, column=5, value=item['unsegregated_expenses'])
                ws.cell(row=r, column=5, value=item['segregated_expenses'])

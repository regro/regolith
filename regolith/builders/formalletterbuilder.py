...
class FormalLetterBuilder(LatexBuilderBase):
    ...
    def latex(self):
        ...
        for letter in self.gtx["formalletters"]:
            ...
            letter["encls"] = [
                f"({string.ascii_lowercase[i]}) {enc[:1].upper()+enc[1:].strip('.')}"
                for i, enc in enumerate(letter["encls"])
            ]
            letter["refs"] = [
                f"({string.ascii_lowercase[i]}) {ref[:1].upper()+ref[1:].strip('.')}"
                for i, ref in enumerate(letter["refs"])
            ]
            # ensure that all paras end in a (single) period
            letter["paras"] = [f"{para.strip('.').capitalize()}." for para in letter.get("paras", "")]
            self.render(
                "naval_correspondence.tex", outfile_name_stub + "_nc.tex", contents=letter, to=nc_to, fr=nc_fr
            )

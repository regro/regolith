...
{% else %}
    ...
    \end{itemize}
{% endif %}
\section*{ Primarily advised Ph.D. students with dissertations in progress }
...
{% else %}
    ...
    \end{itemize}
{% endif %}
\section*{ Other (examination committees, GSAS, CUMC advising etc.) }


\section*{ Postdoctoral researchers and research scientists supported on grants}
{% if len(postdocs) == 0 %}
    {{ None }}
{% else %}
    \begin{itemize}
    ...
    \end{itemize}
{% endif %}
\section*{ Self-funded visitors contributing to research}
{% if len(visitors) == 0 %}
    {{ None }}
{% else %}
    \begin{itemize}
...
{% else %}
    \begin{itemize}
    {% for ugr in dept_service %}
        {% for s in ugr['service'] %}
            \item   {{latex_safe(s['name'])-}}  \\
        {% endfor%}
    {%endfor%}
    \end{itemize}
{% endif %}
